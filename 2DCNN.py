...
# Set the data path
data_path = r"C:\Users\Eric\Desktop\thesis\0612model\cnn2dmodel"
os.chdir(data_path)

# Function to extract raster values
def extract_raster_values(raster_path, shapefile_path, class_column='GEWASGROEP'):
    features = []
    labels = []
    valid_indices = []
    try:
        with rasterio.open(raster_path) as src:
            ...
            shape_data = shape_data.to_crs(raster_crs)
            print("Shapefile data loaded successfully.")
            print("Shapefile CRS:", shape_data.crs)
            print("Shapefile columns:", shape_data.columns)
...
class SatelliteDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        feature = feature.reshape((230, 1, 1))  # Adjust reshaping to match CNN input requirements
        if self.transform:
...
class CNN2D(nn.Module):
    def __init__(self, band_num, class_num):
        ...
        self.Tlayer1 = nn.Sequential(
            nn.Conv2d(self.band_num, channels[0], kernel_size=3, stride=1, padding=1),
...
def save_predictions_to_shapefile(predictions, shapefile_path, output_path, valid_indices):
    try:
        ...
        if len(predictions) != len(shape_data):
            raise ValueError(
                f"Length of predictions ({len(predictions)}) does not match length of filtered shape_data ({len(shape_data)}).")
        ...
        print(f"Error in saving predictions to shapefile: {e}")
...
if __name__ == '__main__':
    band_num = 230
    class_num = 8
    model = CNN2D(band_num, class_num).cuda()
    input_tensor = torch.randn(1, band_num, 1, 1).cuda()
    flops, params = profile(model, inputs=(input_tensor,))
    model.cuda()
    torchsummary.summary(model, (band_num, 1, 1), 1)
    print('flops(G): %.3f' % (flops / 1e+9))
    print('params(M): %.3f' % (params / 1e+6))

    # Extract features and labels for training and testing
    train_features, train_labels, train_valid_indices = extract_raster_values('202208.tif', 'trainingsamples.shp')
    test_features, test_labels, test_valid_indices = extract_raster_values('202208.tif', 'validationsamples.shp')
    ...
    train_output_path = r"C:/Users/Eric/Desktop/thesis/0612model/cnn2dmodel/Predictions08training.shp"
    ...
    test_output_path = r"C:/Users/Eric/Desktop/thesis/0612model/cnn2dmodel/Predictions08test.shp"
